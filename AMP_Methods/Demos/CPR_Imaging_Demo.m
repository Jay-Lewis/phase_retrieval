...
newsize = [28, 28];
ImIn = transpose(reshape(array, newsize));
...
subplot(1,3,2);
imshow(abs(x_hat1));title([denoiser, '-prGAMP Gaussian Measurements']);
subplot(1,3,3);
imshow(abs(x_hat2));title([denoiser, '-prGAMP Coded Diffraction Pattern']);
